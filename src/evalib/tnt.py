...
def train_model(
        ...
        optimizer, epoch,
        enable_l1_loss=False, l1_weight=0.0001):
    model.train()
    train_loss = 0
    train_correct = 0
    train_acc = 0
    ...
    for batch_idx, (data, target) in enumerate(pbar):
        ...
        output_message = '''Epoch: {}, Batch {},
                            Training Loss: {:.8f}
                            Training Accuracy: {:.4f}%'''.format(
                                epoch, batch_idx, train_loss, train_acc)
...
def train_n_test(
        ...
        num_epochs=10, enable_l1_loss=False, l1_weight=0.0001):
    train_acc_history = []
    train_loss_history = []
    val_acc_history = []
    val_loss_history = []
    ...
    for epoch in range(1, num_epochs+1):
        train_acc, train_loss = train_model(model, device,
                                            train_data_loader, optimizer,
                                            epoch, enable_l1_loss, l1_weight)
